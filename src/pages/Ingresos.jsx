...
import { showAlert } from "../utils/alerts"; // Asegúrate de tener este archivo creado
...
export default function Ingresos() {
  ...
  // Helper de estilos para inputs con error
  const inputClass = (hasError) => `
    ...
    ${hasError 
      ...
      : "border-gray-300 bg-gray-50 focus:border-green-400"}
  `;
      ...
      <div className="w-full px-3 sm:px-4 lg:px-6 mx-auto max-w-7xl py-4 sm:py-6 lg:py-8">
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-end bg-white p-4 sm:p-5 rounded-lg sm:rounded-xl border border-gray-200">
            ...
            <button onClick={() => cargarIngresos(1)} className="w-full sm:w-auto h-10 px-4 sm:px-6 rounded-lg bg-blue-600 hover:bg-blue-700 font-medium text-white text-sm sm:text-base transition-colors">Aplicar</button>
          </div>

          {/* CARDS Y TABLA */}
          <SummaryCards totalMes={`$${Number(resumenDatos.totalMes).toLocaleString("es-ES")}`} promedioPorProyecto={`$${Number(resumenDatos.promedioPorProyecto).toLocaleString("es-ES")}`} ultimoIngreso={resumenDatos.ultimoIngreso} />
      ...
      <div className={`fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl border-l transform transition-transform duration-300 z-50 ${mostrarFormulario ? "translate-x-0" : "translate-x-full"}`}>
        <div className="p-6 h-full flex flex-col bg-white">
          ...
          <div className="flex justify-between items-center mb-8 pb-4 border-b">
            ...
            <button onClick={() => setMostrarFormulario(false)} className="text-gray-500 hover:text-gray-700 transition-colors p-1">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <textarea 
                className={`${inputClass(errors.descripcion)} h-auto py-3 resize-none`} 
                rows="4" 
            ...
            <div className="pt-6 border-t flex gap-3">
              <button 
                type="button" 
                onClick={() => setMostrarFormulario(false)} 
                className="flex-1 h-12 rounded-lg bg-white border-2 border-gray-300 text-gray-600 font-bold hover:bg-gray-50 transition-colors"
              >
                Cancelar
              </button>
              <button 
                type="submit" 
                disabled={loading} 
                className="flex-1 h-12 rounded-lg bg-green-500 hover:bg-green-600 font-bold text-white transition-colors disabled:opacity-50 shadow-lg shadow-green-200"
              >
